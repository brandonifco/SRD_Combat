...
played-run half of that commitment is still owed, and **Brandon decided on 2026-09-09
that its report, with #542's verdict, is what lifts `paused:balance-design` from
#306–#311** (the plan's §3 decision 2; the F4 balance issues keep the checkpoint; #708
writes the condition into the framework doc and the `file-issue` skill). His verdict on
the `112ed19` difficulty itself is in: not too hard (decision 3, on #542). Route choice
...
unoffered classes: **build them** (Brandon, 2026-09-09, the plan's §3 decision 4,
overruling the plan's recommendation to cut; #315 tracks the roster, one designer-spec'd
slice per class, the offered-but-thin classes first — the Ranger, #717). **Roles before
counts** (2026-09-09, decision 7, taken; #714): before this phase exits the pool fields
at least one healer or protector, one space controller and one urgent-priority target —
a coverage target for what the pool contains, not the top-down taxonomy #543 ruled out;
each creature still behaves as what it is. Exit: distinct-creature measurement re-run;
the three roles fielded; a property test that every generated encounter resolves.
...
(Linux + Windows), a player-facing README, a tagged release. **A first tester build
is pulled forward, and it is a Windows build** (Brandon, 2026-09-09: "need a windows
build" — the plan's §3 decision 5, taken; #716; Linux alongside on the same pipeline; it
reverses the workflow audit's "Releases/tags now" rejection on new evidence, recorded in
that audit's list) to land after F2's foresight work and before F3's five-fight slice is
