...
**Answers** [#543](https://github.com/brandonifco/SRD_Combat/issues/543) ("Design: how each
monster group should actually fight, from what the creature is"), and three of the five
open questions in [`docs/2026-08-27-enemy-ai-audit.md`](2026-08-27-enemy-ai-audit.md) §6 —
Q1 (is role the right axis), Q3 (should anything flee), and Q5's sequencing blocker. The
...
2026-09-10: eleven he was asked and answered, plus D4, which is a consequence of D3 recorded
separately because a later reader needs it stated outright.
...
last one is the prize — **43 of 330 monsters print immunity to Frightened**, and the list
is exactly right: animated armour, golems, puddings, fungi, gelatinous cubes, plus devas
and balors. Mindless things and proud things do not rout, straight off the printed page,
...
**Amended to a curated list** on the census in §4, which found CR margin anointing an Ogre,
a Spy, a Gargoyle, an Awakened Tree, an Ochre Jelly and a Swarm of Venomous Snakes. The
...
reuses it as the group boundary), and it dodges group-membership entirely — no unit IDs, no
new data structure, no save-format change.
...
2. **A straight Intelligence substitution is backwards in the majority of cases** (55.5%).
   → D10 amended to `max()`.
3. **The bottleneck is composition, not detection.** Curation fires at 13.2%, *no better
   than* CR margin's 15.6%. `EncounterBuilder` draws to a budget from a themed pool and
   simply does not put an officer in most warbands, so **no detection rule can raise the
   leader layer above roughly one fight in seven.** Only changing composition can.

Reading 3 is why D5's amendment accepts 13% rather than chasing it: with most scenarios
hand-authored, **leaders are primarily a scenario feature the author places**, and
procedural fights lean on D9's casualty trigger, which fires regardless. Raising the
procedural rate is filed separately rather than folded in.
...
S1 carries the frozen-transcript regeneration (D3/D4) and is the only slice that does.
...
"CR-margin leader" was scored as: more than one creature present, exactly one at the highest
CR, and that CR at least twice the next-highest present.
