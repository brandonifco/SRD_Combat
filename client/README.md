...
a menu over the board shows neither. **Drawn after the fog and the keyboard cursor's
own ring**, not before: a threatened square is never fogged by construction (the drop
above), so there is nothing left for the fog wash to dim, and the cursor's ring draws
the identical bordered square at the identical width on whatever square it sits on —
drawing the mark first let the cursor's own ring fully occlude it the moment the
keyboard cursor parked on a threatened step. Three-theme legibility (Woodland, Rocky,
