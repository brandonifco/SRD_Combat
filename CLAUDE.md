...
| Pacing | **The median is retired as the headline figure. It has been pinned at 30 of 30 for several slices and can no longer move, so it measures nothing** — a saturated statistic reads exactly the same whether a change helped, hurt, or did nothing. Current `main`, measured 2026-08-16 with `tools/PacingMeasure`, loot on, on **both** ranges: seeds 1–120 read **72 of 120 clearing everything, 93 reaching level 4, 1 dying by fight 4**; seeds 200–320 read **78 of 121, 97, and 6**. **Read the `shape:` line and the per-band lines instead** — `died-by-fight-4` for the opening, `cleared-all` for the ending, and hp-left per band for whether a fight was ever close. **The per-band line is the one that should worry a reader**: party hit points left is 75–81% in *every* band from fights 1–5 to 26–30, while rounds per fight climb 3.0 → 6.0. Fight 27 is not harder than fight 3, it is longer — and the cause is structural rather than a tuning error, because **the ladder is one five-fight cycle played six times** (`index % FightsPerCycle`), so a player has seen the whole run's structure by the end of fight 5. Four slices on 2026-08-16, each measured against a same-build baseline taken immediately before: **movement's printed pass-through clauses** (#203) clears 72 → 76, level-4 86 → 95; **the classic-monster weight** (#204) clears 76 → 66 and died-by-fight-4 11 → 15, a *difficulty* gain from a flavour change, because a classic monster carries more mechanics per XP than an animal; and **resting Long through the opening cycle** (#205) died-by-fight-4 15 → 1 and 14 → 6, clears 66 → 72 and 71 → 78. **That last pair is the structural finding of the day: every fix to the opening inflates the ending**, because both are the same quantity — how many runs survive. The lethal first cycle had been acting as the run's difficulty filter, and removing it exposes that fights 5–30 were never doing that work. Two standing lessons, both earned expensively: **quote a bar you measured yourself, on the build in front of you** — this row previously carried `median 24, 54 clears, 63 at level 4` across several slices while the build actually read 30/72/93, and a number carried in prose is a number nobody re-ran; and **a played run is a first-class instrument, not a nicety** — two partial human sessions found four real bugs and moved the median from 6 to 10.5, where eight slices of deliberate party-power work moved it less, and the 2026-08-16 session produced both of the day's design slices (#204, #205) from complaints no automated measurement could have voiced. Earlier history, kept for its reasoning rather than its numbers: the economy transformed the tail (full clears 2 → 14), #127 spent 2 median deliberately teaching monsters their stat blocks, and every extra die reshuffles each later roll, so seed-set × build interaction swings a 120-seed figure by a few points — measure on two ranges before believing one. |
| Work remaining | **The direction has changed, and the party-power phase (#83) should not be the next thing picked up.** That direction was right when it was chosen: the median was 4–8, runs died at level 1, and the party was demonstrably underpowered against budgets priced for a whole one. **That problem is now fixed** — the opening's death cohort is 1–6 runs of 120 (#205) — and the opposite one is live: **72–78 of 120 runs clear all thirty fights, and hit points left is flat at 75–81% in every band**. Every spell or feature added from here makes an already-easy game easier. The row also used to claim party power was *the only lever that has ever raised pacing*; 2026-08-16 disproved it twice, since a **flavour** change moved difficulty further than any party slice (#204, clears 76 → 66) and a **rest-cadence** change moved the opening further than all of them (#205). What the evidence now points at, in order: **variety and difficulty in the back half** — horde encounters at level 3+ are the strongest candidate, since they are difficulty from *simultaneity* (the one thing the XP budget structurally cannot price, a finding this file has now reached from four directions), they are a different *kind* of fight rather than the same fight with bigger numbers, and they are made of the classic low-CR monsters (#204) that also have sprite art; then **per-cycle variety**, because the ladder is one five-fight cycle six times over; then **a mechanical threat model** (expected damage × accuracy × effective hit points, plus riders and simultaneity) built **as an instrument first and validated against the 120-seed outcomes already on disk** before it is ever allowed to shape an encounter — printed XP stays the budget currency, because print-faithfulness is this project's identity and `ExperienceRules` hangs off it. Still open and unrelated: **`tools/` (5,775 lines of PDF parsing) and `client/` (6,052 lines) have no test project at all** (#189, #190) — a third of the codebase, and the riskiest third. The squad-AI series (#122–#127) is complete: its gains were #123 (focus fire) and #126 (no-healer caution); #124/#125 measured themselves out, and those ladders are required reading before any positional work. |
